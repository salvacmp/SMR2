...
class App:
    def __init__(self, root):
        ...
        #Detection config
        self.model_path = "E:/1_SMR2/SMR2/UR Script/version4.5/bestn.pt"
        self.model = YOLO(self.model_path)


# r        self.perspective_matrix = cv2.getPerspectiveTransform(self.eye_points, self.hand_points)
                            ...
                            if confidence >= 0.77:
                                if True:
                                    ...
                                    if self.PrecissionRequest == False:
                                        ...
                                        # cx, cy = 345,285
                                        # y_camera = (x_center - cx) * depth / fx
                                        # x_camera = (y_center - cy) * depth / fy
                                        # camera_coords = np.array([x_camera, y_camera, 1])
                                        # R = np.array([[1, 0, 0], 
                                        #             [0, 1, 0], 
                                        #             [0, 0, 1]])  
                                        # T = np.array([-0.15170, -0.54743, 105600])  #Robot position

                                        # # Transformation: robot_coords = R @ camera_coords + T
                                        # robot_coordinates = np.dot(R, camera_coords) + T
                                        if 0 <= x_center <= 640 and 0 <= y_center <= 480 and x >= 0 and y >= 0:
                                            ...
                                            calculated_y = x_depth_point - (5.4/100) #movement on x_axis
                                            ...
                                            cv2.putText(
                                                annotated_frame,
                                                f"cX: {robot_coordinates[0]:.5f} | dY: {robot_coordinates[1]:.5f}",
                                        ...
                                        if self.precission_point((x_center, y_center)):
                                            ...
                                            # if True:
                                                ...
                                                if x_center > 0 and y_center > 0 :
                                                    ...
                                                    depth_point = rs.rs2_deproject_pixel_to_point(depth_intrinsic, [int(x_depth_pixel), int(y_depth_pixel)], depth)
                                                    x_depth_point, y_depth_point, z_depth_point = depth_point 
                                                    calculated_y = x_depth_point - (4.28/100) #movement on x_axis
                                                    calculated_x = y_depth_point - (5.85/100)
                                                ...
                                                if self.PrecissionRequest and self.precission_point((x_center, y_center)):
    ...
    def precission_point(self, point):
        ...
        if cam_z > 0.8:
            if class_type == 1:
                polygon = self.generate_polygon(factor=60,class_type=class_type, angle=angle)
            if class_type == 2:
                polygon = self.generate_polygon(factor=20,class_type=class_type, angle=angle)
        else:
            # multiplier = (int(abs(cam_z - 0.8) * 100))*1
            # if multiplier == 0 :
            #     multiplier = 20
            multiplier = 10
            ...
            if class_type == 2:
                polygon = self.generate_polygon(factor=(20+20),class_type=class_type, angle=angle)
